...
async function autoCreateEventDayTasks() {
  ...
  for (const b of bookings) {
    ...
    const templates = [
      { title: `Backup ${eventName}`,             deadline: sameDay, priority: 'high'   },
      { title: `Lightroom ${eventName}`,          deadline: plus3,   priority: 'high'   },
      { title: `Create Thriller for ${eventName}`, deadline: plus3,   priority: 'medium' },
      { title: `Create full video ${eventName}`,   deadline: plus3,   priority: 'medium' },
      { title: `Design Photobook ${eventName}`,    deadline: plus3,   priority: 'medium' },
    ];

    // Skip individual templates if already exist for this booking
