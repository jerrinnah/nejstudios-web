...
(function () {
  ...
  function openLightbox(src, caption) {
    ...
    // Allow inner to grow to fit the image
    inner.style.overflow = 'visible';
    inner.style.maxWidth = 'min(95vw, 1400px)';
  ...
  function closeVideo() {
    ...
    // Reset inner styles set by image lightbox
    const inner = videoModal.querySelector('.video-modal__inner');
    if (inner) { inner.style.overflow = ''; inner.style.maxWidth = ''; inner.style.background = ''; }
    document.body.style.overflow = '';
  }
  ...
  async function saveEventBooking(data) {
    const key = 'nej_bookings';
    // Pull current server list first so we merge, not overwrite
    let existing = [];
    try {
    ...
    if (!existing.find(b => b.id === data.id)) existing.unshift(data);
  ...
  function makeShareUrl(booking) {
    // Short URL: booking-view fetches the record from the server by ID.
  ...
  if (bookingForm) {
    bookingForm.addEventListener('submit', async (e) => {
      ...
      const eventName  = fv('eventName').trim();
      const clientName = eventName;
      const bookingId  = genBookingId();
      const now        = Date.now();

      const budgetRaw   = fv('budget').replace(/[^\d.]/g, '');
      ...
      const booking = {
        ...
        budget:      budgetNum != null && !isNaN(budgetNum) ? budgetNum : fv('budget'),
        deliverables:fv('message'),
        status:      'pending',
        createdAt:   now,
      };

      await saveEventBooking(booking);
