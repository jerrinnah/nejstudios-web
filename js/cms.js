...
function populateAbout() {
  ...
  setVal('about-para3', d.para3 || '');
  setVal('about-image', d.image || '');
  ...
  if (d.image) {
    ...
    if (img) { img.src = d.image; img.classList.add('show'); }
  }

  // File upload handler
  document.getElementById('about-image-file')?.addEventListener('change', async () => {
    const url = await resolveImageUrl('about-image-file', 'about-image');
    if (url) {
      const img = document.getElementById('about-img-preview');
      if (img) { img.src = url; img.classList.add('show'); }
    }
...
function populateLive() {
  const d = get('live');
  setVal('live-title', d.title);
  setVal('live-url', d.url);
  setChecked('live-show-chat', d.showChat !== false);
  setVal('live-logo-url', d.logoUrl);
  setVal('live-logo-pos', d.logoPos || 'top-right');
  setVal('live-logo-size', d.logoSize || 'medium');
  setVal('live-logo-opacity', String(d.logoOpacity ?? 0.9));
  setVal('live-announcement', d.announcement || '');
  setChecked('live-announcement-on', !!d.announcementOn);
  updateLogoPreview();

  document.getElementById('live-logo-url')?.addEventListener('input', updateLogoPreview);
  document.getElementById('live-logo-file')?.addEventListener('change', async () => {
    const url = await resolveLogoUrl('live-logo-file', 'live-logo-url');
    if (url) {
      setVal('live-logo-url', url);
      updateLogoPreview();
    }
  });
}
...
function updateLogoPreview() {
  const url = val('live-logo-url');
  const preview = document.getElementById('live-logo-preview');
  if (!preview) return;
  if (url) {
    preview.src = url;
...
async function saveSection(key) {
  switch (key) {
    ...
    case 'about': {
      const aboutImage = await resolveImageUrl('about-image-file', 'about-image') || val('about-image') || get('about').image || '';
      cmsData.about = {
        ...
        para3: document.getElementById('about-para3')?.value.trim() || '',
        image: aboutImage,
    ...
    case 'live': {
      const logoUrl = await resolveLogoUrl('live-logo-file', 'live-logo-url');
      const opacity = parseFloat(val('live-logo-opacity'));
      cmsData.live = {
        title: val('live-title') || 'Live Event',
        url: val('live-url'),
        showChat: checked('live-show-chat'),
        logoUrl: logoUrl || '',
        logoPos: val('live-logo-pos') || 'top-right',
        logoSize: val('live-logo-size') || 'medium',
        logoOpacity: isNaN(opacity) ? 0.9 : Math.min(1, Math.max(0, opacity)),
        announcement: val('live-announcement') || '',
        announcementOn: checked('live-announcement-on'),
      };
      const fi = document.getElementById('live-logo-file'); if (fi) fi.value = '';
      setVal('live-logo-url', cmsData.live.logoUrl);
      updateLogoPreview();
      break;
    }
    case 'settings': {
...
/**
 * resolveLogoUrl(fileInputId, urlInputId) → Promise<string>
 * Like resolveImageUrl but keeps PNG transparency (logos have alpha).
 * Resized to max 600px on longest side, kept as PNG.
 */
